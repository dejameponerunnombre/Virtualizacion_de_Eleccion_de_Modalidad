...
for($x = 1; $x <= 5; $x++)
{
    ...
    if ($siquesi[0] > 0)
    {
        ...
        {    
            $no="SELECT DNI from eleccion  where ID_Modalidad = $x and Prioridad = 0 and DNI in(SELECT DNI from total)";
            $tas = $conexion -> query($no);
            $sinotas = $tas ->fetch_array();
            if(empty($sinotas)==false)
            {
                $varB = "SELECT t.DNI FROM total t , eleccion e 
                where t.DNI = e.DNI and e.ID_Modalidad = $x and e.Prioridad = 0 and e.Cambio = 'No' 
            ...
            {
                $no="SELECT a.Nombre, e.DNI from eleccion e, alumnos a where e.ID_Modalidad = $x and e.Prioridad = 0 and e.DNI not in(SELECT DNI from total) and e.DNI = a.DNI";
                $tas = $conexion -> query($no);
                $sinotas = $tas ->fetch_array();
                if(empty($sinotas)==false)
                {
                    $var4 = "UPDATE eleccion SET Prioridad = '$y', Situacion = 'Dentro de la modalidad' where DNI = '$sinotas[1]'";
    ...
    if($siquesi[0] > 39)
    {
            ...
            else
        {
            ...
            if(empty($sinotas)==false)
            {
            $var4 = "UPDATE eleccion SET Prioridad = '$y', Situacion = 'En lista de espera' where DNI = '$sinotas[1]'";
            $conn4 = $conexion->query($var4); 
            }
        }
        }
